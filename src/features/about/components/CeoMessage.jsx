      ...
      <Container>
        ...
        <div className="mt-20 grid items-center gap-16 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .6 }}>
            <div className="mb-8 flex h-14 w-14 items-center justify-center rounded-full bg-brand-900 text-white">
              <Quote size={22} />
            </div>
            <blockquote className="text-3xl font-semibold leading-tight text-gray-900">
              {t('quote')}
            </blockquote>
            <p className="mt-8 leading-8 text-gray-600">{t('message')}</p>
            <div className="mt-10 border-l-2 border-yellow-500 pl-5">
              <h4 className="text-lg font-semibold text-gray-900">{t('name')}</h4>
              <p className="mt-1 text-sm uppercase tracking-[0.25em] text-gray-500">{t('role')}</p>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .6, delay: .15 }} className="relative">
            <div className="relative overflow-hidden rounded-3xl">
              <Image src={ceoImage} alt="Chief Executive Officer" className="h-[620px] w-full object-cover" />
            ...
            <div className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-brand-100" />
          </motion.div>
        </div>
      </Container>
    </section>
